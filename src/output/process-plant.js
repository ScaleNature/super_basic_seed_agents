...
async function processPlant(genus, species) {
  ...
  // Step 1: Gather plant data (validation + native check)
  console.log(`Step 1: Gathering data for "${genus} ${species}"...`);
  ...
  console.log(`  Genus: ${plantRecord.genus}`);
  console.log(`  Species: ${plantRecord.species}`);
  console.log(`  Family: ${plantRecord.family}`);
  console.log(`  Native to SE Michigan: ${plantRecord.isNative}`);
  if (plantRecord.nativeCheckNotes) {
    console.log(`  Notes: ${plantRecord.nativeCheckNotes}`);
  }
  console.log();
